...
# transpose angle to trigger Julia's broadcasting mechanism https://docs.julialang.org/en/v1/manual/arrays/#Broadcasting
x = radius .* cos.(angle')
y = radius .* sin.(angle')
z = parabola(x, y)
...
#tangentPoint=[0.5, 0.5]

# partial derivative w.r.t. x
directionalbox!(ax, parabola, tangentPoint, :x, tangentColor=GLMakie.Colors.HSL(0, 1, 0.4))

# partial derivative w.r.t. y
directionalbox!(ax, parabola, tangentPoint, :y, tangentColor=GLMakie.Colors.HSL(125, 1, 0.4))

# directional derivative example with arbitrary direction 
#directionalbox!(ax, parabola, [-.2, -.2], [-1.0, 1.0], boxLimits=(-0.3, 0.3))
